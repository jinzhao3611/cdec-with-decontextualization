...
import collections
import click
from cdec_modeling.load_as_ecb_doc import prepare_ecb_docs
from cdec_modeling import KEYWORD, SOURCE, TOPIC_DOCS
            ...
            if cluster_1 != cluster_2:
                if is_train:
                    ...
                    if (cluster_1, cluster_2, q) \
                        ...
                        if add_to_training:
                            pairs.add((cluster_1, cluster_2, q))
                        test_pairs.add((cluster_1, cluster_2))
                else:
                    if (cluster_1, cluster_2) not in pairs and \
                            (cluster_2, cluster_1) not in pairs:
                        pairs.add((cluster_1, cluster_2))
...
def topic_to_mention_list(topic, data, dataset):
    event_mentions = []
    entity_mentions = []
            ...
            for sen_id in sorted(cur_doc.sentences.keys()):
                ...
                for mention in mentions:
                    ...
                    # 	coref_chain = '-'
                        ...
                        # NOTE: remove singleton from ECB?
                        if cluster_size[topic_id][mention.cd_coref_chain] >= 1:
                            coref_chain = mention.cd_coref_chain
                        else:
                            coref_chain = '-'
                    ...
                    if dataset == 'ecb':
                        ...
                        out_coref.write('{}\t({})\n'.format(mention.mention_id, coref_chain))
...
def write_event_coref_results(corpus, out_dir, dataset, alpha, threshold):
    out_file = os.path.join(out_dir, f'CD_test_event_mention_based_{alpha}_{threshold}.response_conll')
    write_mention_based_cd_clusters(corpus, is_event=True, is_gold=False, out_file=out_file, dataset=dataset)


def test_models(dataset, test_set, write_clusters, out_dir, alpha, threshold):
    '''
    ...
    with torch.no_grad():
        for topic_id, topic in enumerate(topics):
            ...
            event_mentions, entity_mentions = topic_to_mention_list(topic, test_set, dataset=dataset)

            print('Events count: ', len(event_mentions))
            ...
            merge_cluster(topic_event_clusters, cluster_pairs, epoch, topics_counter, topics_num, threshold, alpha,
                          True)

            set_coref_chain_to_mentions(topic_event_clusters, is_event=True, is_gold=True, intersect_with_gold=True)

        if write_clusters:
            write_event_coref_results(test_set, out_dir, dataset, alpha, threshold)
...
def run_conll_scorer(isGold, gold_file_path, out_dir, alpha, threshold):
    if isGold:
        event_response_filename = os.path.join(out_dir,
                                               f'CD_test_event_mention_based_{alpha}_{threshold}.response_conll')
    ...
    event_conll_file = os.path.join(out_dir, f'event_scorer_cd_out_{alpha}_{threshold}.txt')
    ...
    scores_file = open(os.path.join(out_dir, f'conll_f1_scores_{alpha}_{threshold}.txt'), 'w')
...
# @click.command()
# @click.argument('model_out_file', type=click.Path(exists=True))
# @click.argument('out_dir', type=click.Path())
def main(model_out_file):
    test_data = prepare_ecb_docs(KEYWORD.rh, SOURCE.fd, use_decont=True)
    ...
    out_dir = DECONT_ARTICLE_PATH

    for i in alpha:
        for j in threshold:
            print("alpha: {}, threshold: {}".format(i, j))

            with open(model_out_file, 'r') as entailment_file:
            ...
            print(len(entailment_score))

            test_models('ecb', test_data, write_clusters=True, out_dir=str(out_dir), alpha=i,
                        threshold=j)
...
if __name__ == '__main__':
    ...
    model_out_file = DECONT_ARTICLE_PATH / "federalist_scores_00_01_dummy_test.txt"
    main(model_out_file)
